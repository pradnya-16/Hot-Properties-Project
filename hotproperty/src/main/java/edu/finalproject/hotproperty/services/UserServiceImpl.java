  ...
  public boolean updateUserProfile(String currentEmail, UserProfileUpdateDto profileUpdateDto) {
    ...
    if (newEmail != null && !newEmail.trim().isEmpty() && !newEmail.equalsIgnoreCase(user.getEmail())) {
      if (userRepository.existsByEmail(newEmail)) {
        log.warn("Attempt to update email to an existing one: {} by user {}", newEmail, currentEmail);
    ...
    log.info("User profile updated for (original email): {}. Email changed: {}", currentEmail, emailChanged);
    return emailChanged;
  }
